...
def Shop(request, dishid):
    ...
    Cartdish,total = Total(request.user)
    d={'dish':dish, 'total':total,'Cartdish':Cartdish}
    return render(request,'shop.html',d)

def add_to_cart(request,dishid):
...
def editTeam(request):
    ...
    if 'add' in request.POST:
        ...
        img = request.FILES['img']
        Team.objects.create(name=name,designation=des,fb=fb,twitter=twt,insta=insta,img=img)
    if 'delete' in request.POST:
